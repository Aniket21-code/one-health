...
S1En.grid(row=7, column=4)

S2En = OptionMenu(root, Symptom2,*OPTIONS)
S2En.grid(row=8, column=4)

S3En = OptionMenu(root, Symptom3,*OPTIONS)
S3En.grid(row=9, column=4)

S4En = OptionMenu(root, Symptom4,*OPTIONS)
S4En.grid(row=10, column=4)

S5En = OptionMenu(root, Symptom5,*OPTIONS)
S5En.grid(row=11, column=4)
...
def main():
    ...
    S1Lb = st.text_input ("Variance","Type Here") 
    S2Lb= st.text_input("skewness","Type Here")
    S3Lb = st.text_input("curtosis","Type Here") 
    S4lb= st.text_input ("entropy","Type Here")
    result=""
    if st.button("Predict"):
          result-predict_note_authentication (S1Lb,S2Lb,S3Lb,S4Lb) 
